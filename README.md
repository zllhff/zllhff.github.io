...
- **2026-07-09** — **Floe** als 6. Kachel im Root-Index (`assets/js/apps-data.js`), dunkles Ice/Ember-Brand-Theme, Icon (`assets/icons/floe.png`) + 5 DE/EN-Screenshots (`assets/screenshots/floe/{de,en}/`) aus `Floe/store_assets` (= die in ASC geladenen), `storeUrl: null` → „Coming soon" (noch nicht live). Header-String „Fünf → Sechs ruhige iOS-Apps". Tests auf 6 Apps + zwei Non-Store-Apps (vow, floe) angepasst. — **Impressum-Adress-Normalisierung (rechtlich):** volle ladungsfähige Anschrift `Luegstraße 22, 44267 Dortmund` jetzt konsistent auf ALLEN Impressen (Root + faro/roll/kept/shin/vow/floe) sowie `roll/privacy-de` (Verantwortlicher). Vorher zeigten die App-Subpages nur „Dortmund" (nicht § 5 DDG-konform). `shin/impressum.html` trug noch die alte Adresse „Benninghofer Heide 49" → korrigiert. Kein Telefon (nicht Pflicht), Mail überall `ahoi@sent.com`. Offen (Otto-Todoist-Task): Wechsel auf eine c/o-Geschäftsadresse via impressumservice.de, damit die Privatadresse raus kann.
